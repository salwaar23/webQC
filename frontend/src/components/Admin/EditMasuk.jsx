...
const EditMasuk = () => {
    ...
    const updateMasuk = async (e) => {
        e.preventDefault();
        try {
            await axios.patch(`http://localhost:5000/Masuk/${id}`, {
                kode_barang,
                nama_barang,
                jumlah,
                kualitas,
    ...
    const getMasukById = async () =>{
        const response = await axios.get(`http://localhost:5000/Masuk/${id}`);
        ...
        settgl(response.data.tgl_masuk);
    }

    return(
        <div>
            <div className="text-center mt-10 text-2xl">Edit Data Barang Masuk PT. MDS Textile</div>
            ...
            <div><form onSubmit={updateMasuk}><div className="inline-flex mt-[40px] ml-[80px]">
                <div className="">
            ...
            <button type="submit" className="bg-blue-500 w-24 h-10 text-xl text-white text-center rounded-md pt-[6px] ml-[360px] mt-10">Simpan</button>
            </form>
            </div>
        </div>
    )
